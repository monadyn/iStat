...
def main():
	...
	OUTFILE.write("date_time,HTTP,AJP,MySQL,HTTP(over 1s),AJP(over 1s),MySQL(over 10ms),MySQL(over 100ms),MySQL(over 1sec)\n")
	...
	for target_time in range(int(stime_epoch), int(etime_epoch) + 1):
		for ms in range(0, multi_count_in_sec):
			...
			OUTFILE.write(",%f,%f,%f" % (HTTP_multi[ms_time], AJP_multi[ms_time],
						     MYSQL_multi[ms_time]))
			OUTFILE.write(",%f,%f,%f,%f,%f" % 
				      (HTTP_multi_1sec[ms_time], AJP_multi_1sec[ms_time],
	...
	# write headers on output files
	OUTFILE2.write("date_time,HTTP,AJP,MySQL(starting time),MySQL(finishing time)\n")
	for target_time in range(int(stime_epoch), int(etime_epoch) + 1):
		for ms in range(0, multi_count_in_sec):
			ms_time = target_time * multi_count_in_sec + ms
	                ms_time_f = float(ms_time) / multi_count_in_sec
			...
			OUTFILE2.write(",%f,%f,%f,%f" % (HTTP_RT, AJP_RT, MYSQL_RT, MYSQL_RT2))
	                OUTFILE2.write("\n")
	OUTFILE2.close()
	...
	# write headers on output files
	OUTFILE3.write("date_time,HTTPstart,AJPstart,MySQLstart(all),MySQLstart(10ms),MySQLstart(100ms),MySQLstart(1s),")
	...
	for target_time in range(int(stime_epoch), int(etime_epoch) + 1):
		for ms in range(0, multi_count_in_sec):
			...
			OUTFILE3.write(",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d" % 
				       (HTTP_input[ms_time], AJP_input[ms_time],
